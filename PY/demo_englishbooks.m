...
for i = 1:thin:length(results.eta)
    fprintf(1, '%d\n', i);
    W_pred = results.eta(i)*pystickbreaking(results.eta(i), results.sigma(i), n);
    ...
    pi_post = [pi_post ; pypoststickbreaking(cnts, results.eta(i), results.sigma(i))];
end
...
%%
save(['englishbooks ', datestr(datetime('now'))]);
