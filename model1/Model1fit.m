function results = Model1fit(cnts, n_samples)

% initialize parameters
[ty, u, eta, sigma, ~, tau] = Model1init(cnts);
% fix c
c = 1;
log_u = log(u);
log_eta = log(eta);
...
fprintf(1, 'initial estimates: u %f eta %f, sigma %f, tau %f\n',...
    u, eta, sigma, tau);
...
for i = 1:n_samples
    % resample ty
    U = @(x)(-Model1augloglikel(cnts, x, u, eta, sigma, c, tau));
    grad_U = @(x)(-Model1augloglikel_grad(x, cnts, u, sigma, c, tau));
    ty = hmc_step(ty, U, grad_U, eps, L);

    % resample eta
    log_eta_new = log_eta + 0.01*randn;
    eta_new = exp(log_eta_new);
    log_r = Model1augloglikel(cnts, ty, u, eta_new, sigma, c, tau) ...
        - 0.5*log_eta_new^2 ...
        - Model1augloglikel(cnts, ty, u, eta, sigma, c, tau) ...
        + 0.5*log_eta^2;
    ...
    log_r = Model1augloglikel(cnts, ty, u, eta, sigma_new, c, tau_new) ...
    ...
    log_r = Model1augloglikel(cnts, ty, u, eta, sigma, c, tau_new) ...
        - 0.5*log_delta_new^2 ...
        - Model1augloglikel(cnts, ty, u, eta, sigma, c, tau) ...
        + 0.5*log_delta^2;
    if rand < exp(min(log_r, 0.0))
    ...
    u_new = exp(log_u_new);
    log_r = Model1augloglikel(cnts, ty, u_new, eta, sigma, c, sigma+delta) ...
        - 0.5*log_u_new^2 ...
        - Model1augloglikel(cnts, ty, u, eta, sigma, c, sigma+delta) ...
        + 0.5*log_u^2;
    if rand < exp(min(log_r, 0.0))
        log_u = log_u_new;
        u = u_new;
    ...
    ll = Model1augloglikel(cnts, ty, u, eta, sigma, c, tau);
    if i > burn_in
    ...
    if (i > burn_in) && (mod(i,thin) == 0)
        ...
        results.thinned_y = [results.thinned_y; c./(1 + exp(-ty))];
    end
end
end
